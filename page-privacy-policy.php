...
<div role="main">
    <div class="row">
            <div class="main-content">
                ...
                <div class="privacy-content" style="max-width: 960px; margin: 0 auto; padding: 20px;">
                <?php if ( function_exists('yoast_breadcrumb') ) {
                    yoast_breadcrumb('<p id="breadcrumbs">','</p>');
                    } ?>
                <h1 class="page-header entry-title">Privacy Policy</h1>
                    <h3><?= do_shortcode('[client_info field="office_name"]'); ?> Internet Privacy & Security Policy</h3>
                    <p><?= do_shortcode('[client_info field="office_name"]'); ?> respects your privacy and is committed to protecting sensitive information at all times. This online privacy statement explains how <?= do_shortcode('[client_info field="office_name"]'); ?> collects, uses and safeguards information on <?= "site_url"; ?>. This Internet Privacy Statement applies ONLY to information collected by the <?= do_shortcode('[client_info field="office_name"]'); ?> through its website.</p>
                    ...
                    <p>If you have questions about this Internet Privacy Statement, please send us an email to <?= do_shortcode('[client_info field="email"]'); ?>.</p>
                    <h3>Information We Collect and How We Use It</h3>
                    <p><b>Information we collect —</b> When you browse <?= "site_url"; ?> and do not interact with the site for any online service or product from <?= do_shortcode('[client_info field="office_name"]'); ?>, you browse anonymously. Personally identifiable information–such as your name, address, phone number and email address–is not collected as you browse.</p>
                    ...
                    <ul>
                        ...
                        <li>Deliver <?= do_shortcode('[client_info field="office_name"]'); ?> services such as newsletters, meetings or events</li>
                        <li>Notify you about new products/services, special offers, upgrades and other related information from the <?= do_shortcode('[client_info field="office_name"]'); ?></li>
                    </ul>
                    <p><b>How we use your email address —</b> Users who have opted in to our electronic newsletter may periodically receive emails from us regarding new products/services, special offers, news and other related information from <?= do_shortcode('[client_info field="office_name"]'); ?>.</p>
                    ...
                    <p><b>How to opt out of email —</b> To opt out of an email list, please contact <?= do_shortcode('[client_info field="office_name"]'); ?>, <?= do_shortcode('[client_info field="current_customer_number"]'); ?>.</p>
                    <p><b>Children under 13 —</b> We do not knowingly solicit data online from or market online to children under the age of 13.</p>
                    <p>Information security — <?= do_shortcode('[client_info field="office_name"]'); ?> implements security measures to protect against unauthorized access to or unauthorized alteration, disclosure or destruction of data. We restrict access to personal information to our business partners who may need to know that information in order to operate, develop or improve our services. These individuals are bound by confidentiality obligations and may be subject to discipline, including termination and criminal prosecution, if they fail to meet these obligations.</p>
                    <h3>How we safeguard information</h3>
                    <p><b>Site security features —</b> <?= do_shortcode('[client_info field="office_name"]'); ?> takes extensive measures and carefully selects third party companies that have sound security practices in place. Please be aware that no data transmission over the Internet can be guaranteed 100% secured. For these reasons, and despite our measures, we cannot guarantee or warrant the security of any information transmitted to us electronically.</p>
                    <p>Our Internet Privacy Statement refers only to information collected through our website. For a copy of our HIPAA Notice of Privacy Practices, please contact <?= do_shortcode('[client_info field="office_name"]'); ?>, <?= do_shortcode('[client_info field="current_customer_number"]'); ?>.</p>
                    <p><b>Linking to other Internet sites —</b> <?= do_shortcode('[client_info field="office_name"]'); ?> website may link to other Internet sites as other Internet sites may link to <?php get_site_url(); ?>. <?= do_shortcode('[client_info field="office_name"]'); ?> is not responsible for the privacy practices or content of other websites and cannot monitor them. To ensure your privacy is protected, we recommend that you review the privacy statements of other Internet sites you visit.</p>
                    <h3>Contacting Us About Privacy</h3>
                    <p>You may contact us about our privacy practices at any time:<br/>Email: <?= do_shortcode('[client_info field="email"]'); ?><?= do_shortcode('[client_info field="email"]'); ?><br/>Phone: <?= do_shortcode('[client_info field="office_name"]'); ?></p>
                    <p>By Mail At: <br/><?= do_shortcode('[client_info field="office_name"]'); ?>, <br/><?= do_shortcode('[client_info field="street_address"]'); ?> <br/><?= do_shortcode('[client_info field="city"]'); ?>, <?= do_shortcode('[client_info field="state"]'); ?> <?= do_shortcode('[client_info field="zip"]'); ?></p>
